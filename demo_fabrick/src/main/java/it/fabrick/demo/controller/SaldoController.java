...
import it.fabrick.demo.dto.ResponseSaldoExended;
...
public class SaldoController implements DemoController {
    ...
    @Autowired
    public SaldoController(RestTemplateBuilder builder) {
        this.restTemplate = builder.build();
    }
	
	@ApiOperation(value = "Get Saldo by accountId")
	@RequestMapping(path = "/{accountId}", method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
    public String getSaldoByIdAccount(@PathVariable String accountId) throws JsonMappingException, JsonProcessingException{
	    ...
	    ResponseSaldoExended respobj = objectMapper.readValue(response.getBody(), ResponseSaldoExended.class);

	    if(response.getStatusCode() == HttpStatus.OK) {
	        return respobj.getPayload().getBalance().toString();
	    } else {
	        return null;
	    }
